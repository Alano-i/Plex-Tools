...
def get_douban_top250_tmdb_list():
    ...
    for movie in douban_top250:
        ...
        for i in range(5):
            ...
            if tmdb_id:
                ...
                _LOGGER.error(f"{plugins_name} 第 {i+1}/5 次获取{movie} 的 tmdb_id 失败")
    ...
    if douban_top250_tmdb_list:
        if old_douban_top250_list != douban_top250_tmdb_list and len(douban_top250_tmdb_list) == 250:
            ...
            _LOGGER.info(f'{plugins_name}最新「豆瓣TOP250」列表与缓存相同，或获取不足250部，不更新缓存，共 {len(douban_top250_tmdb_list)} 部电影，如下：\n{douban_top250_tmdb_list}')
    else:
...
def get_imdb_top_250():
    ...
    if imdb_ids:
        if old_imdb_top250_list != imdb_ids and len(imdb_ids) == 250:
            ...
            _LOGGER.info(f'{plugins_name}最新「IMDB TOP250」列表与缓存相同，或获取不足250部，不更新缓存，共 {len(imdb_ids)} 部电影，如下：\n{imdb_ids}')
    else:
